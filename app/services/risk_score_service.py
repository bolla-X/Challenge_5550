...
def compute_risk_score(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    alerts = Alert.query.filter(Alert.last_seen_at >= window_start).all()
    ...
    worst_key = max(FEATURES, key=lambda key: (features_payload[key]["score"], features_payload[key]["alert_count"]))
    overall = {
...
def compute_risk_trend(*, hours: int = 24, bucket_hours: int = 1, now: datetime | None = None) -> dict[str, Any]:
